...
Vagrant.configure("2") do |config|
      ...
      # Cấu hình máy ảo đầu tiên
  config.vm.define "vm1" do |vm|
    vm.vm.box = "ubuntu/bionic64"

    # Chia sẻ thư mục giữa máy host và máy ảo
    vm.vm.synced_folder ".", "/vagrant", type: "virtualbox"  # tránh dùng NFS trên Win

    # Thiết lập mạng riêng
    vm.vm.network "private_network", ip: "192.168.56.10"

    # Provision: cài đặt phần mềm cần thiết
    vm.vm.provision "shell", inline: <<-SHELL
      apt-get update
      apt-get install -y htop
    SHELL

    # Cấu hình bộ nhớ máy ảo
